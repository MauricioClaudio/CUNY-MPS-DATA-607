...
athlete.dat=read.csv("https://raw.githubusercontent.com/MauricioClaudio/CUNY-MPS-DATA-607/main/Untidy%20data%20set%201.csv")
athlete.dat
```
  
We'll tidy up the table by reducing it from five to three columns using the ``pivot_longer`` function. In the process, we'll change the name of the column corresponding to the Month to *TimePeriod* and name the column resulting column of values *Time*. Likewise, we will convert the values of *TimePeriod* and *Time* to integers so that they can be compared and analyzed numerically downstream.  
...
energy.dat=read.csv("https://raw.githubusercontent.com/MauricioClaudio/CUNY-MPS-DATA-607/main/Untidy%20dataset%202.csv")
energy.dat
```
...
## 4. Two variables, two values in one column. Good Grief!    
...
Visually on the scatter plot and on QQ-norm plot, the bottom three values corresponding to study time of less than 10 minutes don't look quite right. They look like outliers spoiling the stew. Let's eliminate them from the dataset and run the scatter plot, regression and QQ norm plot once again.  
