...
### So who are the strongest players, really?
Player points alone don't tell the whole story.
A player may have accumulated points by playing against weak
opponents. Conversely, a player with few points may actually be
stronger than what his/her points would suggest because s/he has
played against strong opponents. Multiplying player points by the
average opponent score can give a fuller picture of player strength.
Let's take a look at that...

```{r}
reslt$POINTS=as.numeric(reslt$POINTS)
reslt$strength=round(reslt$POINTS*reslt$PRERATING/1000,1)
reslt2=reslt[order(-reslt$strength),]
reslt2
```

The calculated variable ``strength`` now shows us that the previous
ranking based on points alone may be misleading. Player rankings now
show a different pattern with two players previously in the top-10,
players 9 and 10, dropping to 13th and 14th place respectively.
A top-10 ranking by ``strength`` is as follows:

```{r}
reslt2=reslt2[1:10,c("NAME","POINTS","strength")]
reslt2
```
  
We might also ask, "who are the most improved players?" or which players have shown the biggest increases between their Pre-rating and Post-rating scores. Let's find out who the top-10 most improved players are by extracting the Post-rating and comparing it to the previously extracted Pre-rating...  
