...
## 1. Untidy data: Values as variables in column headers  
...
## 4. Good Grief! Two variables, two values in one column  

Frequently, we get tables that make you ask, *What were they thinking? Or in the words of John McEnroe, 'you cannot be serious!'* One such table is one where there not one, but two variables in a column, and consequently, two values in the column cells. Let's take a look at an example of such a table posted by Eric. L.  
...
Now that the data is tidy we can begin to probe it. Is there a relationship between study time and test score? Let's look at the scatter plot.  
...
cat("Correlation coefficient of the two variables:",c[1,2])
  ...
  cat("The regression equation is:\n",
      "Y =",
      round(coef(lm(testing.tidy$TestScore~testing.tidy$StudyMinutes))["(Intercept)"]),
      "+ (",
      round(coef(lm(testing.tidy$TestScore~testing.tidy$StudyMinutes))["testing.tidy$StudyMinutes"],2),
      "* X)"
      )
```
  
Or, how are study times and test scores distributed by Gender?
